...
| Rule 경로 | 413 (95.8%) |
| **LLM 경로** | **18 (4.2%)** |
| 처리 시간 | 약 10분 (LLM 호출 포함) |

### 실패 원인 분류
- **429 Rate Limit (rule fallback 처리)**: 47개 파일 → rate limit 재시도 로직 없어 일부 rule fallback으로 흡수됨 (→ `run_llm_retry.py` 로 재처리 예정)
- **unknown version 파싱 실패**: 5~29개 (PDF 포맷 비정형 - 버전 식별 불가)
